...
import { Radio, ChevronDown, CheckCircle2, AlertTriangle, ExternalLink, Phone, Mail, HelpCircle, ArrowRight } from 'lucide-react';
import { useUser } from '../../context/UserContext';

export default function IndexBasedLivestockInsurance() {
    ...
    const highlights = [
        "Proactive intervention before asset mortality occurs",
        "Continuous high-fidelity satellite pasture telemetry",
        "Covers aggregate herd scale concurrently",
        "Enables immediate targeted emergency response liquidity"
    ];

    const benefits = [
        "Prevents total herd loss cycles before degradation sets in",
        "Fosters institutional community capacity building",
        "Enables predictable long-term income smoothing profiles",
        "Directly safeguards vulnerable pastoral livelihoods"
    ];

    const limitations = [
        "Strictly contingent on consistent, precise satellite vegetation indexes",
        "Potential vulnerability to false anomaly triggers in uncalibrated zones",
        "Functionally confined to pastoral and agro-pastoral geography layers",
        "Inherent basis risk if localized rangeland dynamics vary from macro data"
    ];

    const providers = [
        { name: "ARC (African Risk Capacity) - IBLI Specialist", phone: "+234 803 IBLI", url: "https://www.arcweather.org/ibli", email: "ibli@arcweather.org" },
        { name: "NAIC Livestock Division (National Agricultural Insurance)", phone: "+234 800 NAIC", url: "https://naic.gov.ng", email: "livestock@naic.gov.ng" }
    ];
        ...
        >
            ...
            <motion.div variants={itemVariants} className="text-center max-w-2xl mx-auto pt-6 mb-4">
                ...
                <p className={`text-sm tracking-wide font-medium ${textSecondary}`}>
                    Satellite-Derived Range Biomass Protection Layer
                </p>
                ...
                <div className={`md:col-span-2 border rounded-3xl p-8 flex flex-col justify-between transition-all ${cardBg}`}>
                    <div>
                        ...
                        <p className={`text-sm leading-relaxed font-normal ${textSecondary}`}>
                            IBLI completely bypasses traditional carcass validation paradigms by indexing directly to the structural health of local grazing geography. Utilizing real-time orbital scanning arrays, the system calculates the exact volume and density profiles of accessible forage, triggering defensive fiscal liquidations before animals face physical starvation risks.
                        </p>
                    </div>
                </div>
                ...
                <div className={`rounded-3xl border-2 p-8 flex flex-col justify-between transition-all ${
                    ...
                    <div>
                        ...
                        <p className={`text-xs leading-relaxed font-medium ${theme ? "text-slate-700" : "text-slate-300"}`}>
                            Deploys proactive emergency operational capital precisely when pasture density breaks safe parameters. This unlocks immediate capability to buy supplementary feed, transport emergency water trucks, or organize migrations before herd degradation spirals.
                        </p>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-blue-500 mt-4">
                        Orbital Array Engaged <ArrowRight size={10} />
                    </div>
                </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: "Trigger Parameter", val: "Forage < Critical Threshold" },
                    { label: "Payout Velocity", val: "3–7 Days" },
                    { label: "Product Class", val: "Satellite / Parametric" },
                    { label: "Base Premium", val: "₦5,000 / ssn" }
                ].map((metric, idx) => (
                        ...
                        >
                            <div className={`p-8 border-t text-xs space-y-6 ${borderStyle} ${theme ? "bg-white" : "bg-slate-950/20"}`}>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Protected Assets</span> <p className={`font-bold text-sm ${textPrimary}`}>200 Goats (Pastoral)</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Guaranteed Cap</span> <p className={`font-bold text-sm ${textPrimary}`}>₦150,000</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Trigger Status</span> <p className={`font-bold text-sm ${textPrimary}`}>Forage &lt; Historic Floor</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Scanning Mode</span> <p className={`font-bold text-sm ${textPrimary}`}>Orbital Spectral NDVI</p></div>
                                </div>
