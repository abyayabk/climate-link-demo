...
import { Thermometer, ChevronDown, CheckCircle2, AlertTriangle, ExternalLink, Phone, Mail, HelpCircle, ArrowRight } from 'lucide-react';
import { useUser } from '../../context/UserContext';

export default function TemperatureInsurance() {
    ...
    const highlights = [
        ...
        "Covers heatwaves and frosts",
        "Satellite temperature monitoring",
        "Protects during vulnerable periods"
    ];

    const benefits = [
        "Protects in increasingly hot climates",
        "Covers unexpected cold snaps",
        "Automatic payout on temperature data",
        "No need for loss documentation"
    ];

    const limitations = [
        "Basis risk if microclimates vary across regions",
        "May not perfectly match all specific crop phenology",
        "Regional temperature grid stations may be distant from remote fields"
    ];
        ...
        >
            ...
            <motion.div variants={itemVariants} className="text-center max-w-2xl mx-auto pt-6 mb-4">
                ...
                <p className={`text-sm tracking-wide font-medium ${textSecondary}`}>
                    Thermal Stress Parametric Microinsurance Layer
                </p>
                ...
                <div className={`md:col-span-2 border rounded-3xl p-8 flex flex-col justify-between transition-all ${cardBg}`}>
                    <div>
                        ...
                        <p className={`text-sm leading-relaxed font-normal ${textSecondary}`}>
                            This mechanism mitigates thermal volatility by anchoring risk directly to regional atmospheric temperatures. Utilizing high-fidelity satellite observation telemetry, the platform automatically flags extreme anomalies—such as prolonged heatwaves or deep frosts. Payouts execute immediately upon breach parameters, bypassing traditional field-loss validation timelines entirely.
                        </p>
                    </div>
                </div>
                ...
                <div className={`rounded-3xl border-2 p-8 flex flex-col justify-between transition-all ${
                    ...
                    <div>
                        ...
                        <p className={`text-xs leading-relaxed font-medium ${theme ? "text-slate-700" : "text-slate-300"}`}>
                            Delivers unalterable, data-driven security against climate extremes. By protecting vulnerable vegetative cycles from thermal shock, producers lock down liquidity before crop degradation becomes irreversible.
                        </p>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-blue-500 mt-4">
                        Thermal Guard Enabled <ArrowRight size={10} />
                    </div>
                </div>
            </motion.div>
                        ...
                        >
                            <div className={`p-8 border-t text-xs space-y-6 ${borderStyle} ${theme ? "bg-white" : "bg-slate-950/20"}`}>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Sample Crop Scope</span> <p className={`font-bold text-sm ${textPrimary}`}>1.5 Ha (Vegetables)</p></div>
                                    ...
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Trigger Condition</span> <p className={`font-bold text-sm ${textPrimary}`}>&gt; 38°C for 7+ Days</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Telemetry Source</span> <p className={`font-bold text-sm ${textPrimary}`}>Orbital Thermal Sensor</p></div>
                                </div>
            ...
            {/* AUTHORIZED CHANNELS (Single Provider Clean Layout Mapping) */}
