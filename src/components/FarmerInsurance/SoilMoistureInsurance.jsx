...
import { Droplets, ChevronDown, CheckCircle2, AlertTriangle, ExternalLink, Phone, Mail, HelpCircle, ArrowRight } from 'lucide-react';
import { useUser } from '../../context/UserContext';

export default function SoilMoistureInsurance() {
    ...
    const highlights = [
        "Catches hidden drought early",
        "Pre-loss cash protection",
        "Satellite & sensor monitoring",
        "Triggers before visible damage"
    ];

    const benefits = [
        "More precise than rainfall-only insurance",
        "Better for high-evaporation zones",
        "Timely intervention cash",
        "Captures invisible drought stress"
    ];

    const limitations = [
        "Requires sensor network in area",
        "May have false triggers in some weather conditions",
        "Satellite data telemetry resolution varies by region"
    ];

    const providers = [
        { name: "NAIC (Nigerian Agricultural Insurance Corporation)", phone: "+234 800 NAIC", url: "https://naic.gov.ng", email: "info@naic.gov.ng" },
        { name: "Arc Ltd (Satellite Monitoring & Telemetry)", phone: "+234 803 ARCSAT", url: "https://www.arcweather.org", email: "sensors@arcweather.org" }
    ];
        ...
        >
            ...
            <motion.div variants={itemVariants} className="text-center max-w-2xl mx-auto pt-6 mb-4">
                ...
                <p className={`text-sm tracking-wide font-medium ${textSecondary}`}>
                    Pre-Loss Adaptive Climate Risk Finance Tool
                </p>
                ...
                <div className={`md:col-span-2 border rounded-3xl p-8 flex flex-col justify-between transition-all ${cardBg}`}>
                    <div>
                        ...
                        <p className={`text-sm leading-relaxed font-normal ${textSecondary}`}>
                            This architecture targets the raw water saturation profile within the subterranean crop boundary layer. Whenever real-time regional soil matrix hydration drops beneath operational target thresholds, parametric triggers generate an immediate payout—releasing critical capital to the producer before catastrophic wilting sets in.
                        </p>
                    </div>
                </div>
                ...
                <div className={`rounded-3xl border-2 p-8 flex flex-col justify-between transition-all ${
                    ...
                    <div>
                        ...
                        <p className={`text-xs leading-relaxed font-medium ${theme ? "text-slate-700" : "text-slate-300"}`}>
                            Acts as an agile early-warning circuit. By capturing baseline index variations through high-precision orbital remote sensing arrays, you capture liquidity before thermal stress permanently alters seasonal yields.
                        </p>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-blue-500 mt-4">
                        Proactive Protection <ArrowRight size={10} />
                    </div>
                </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: "Trigger Threshold", val: "Moisture < 20%" },
                    { label: "Payout Velocity", val: "1–3 Days" },
                    { label: "Product Class", val: "Index-Based" },
                    { label: "Est Premium", val: "Rules Vector Matrix" }
                ].map((metric, idx) => (
                        ...
                        >
                            <div className={`p-8 border-t text-xs space-y-6 ${borderStyle} ${theme ? "bg-white" : "bg-slate-950/20"}`}>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Sample Crop Scope</span> <p className={`font-bold text-sm ${textPrimary}`}>1.5 Ha (Maize)</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Guaranteed Cap</span> <p className={`font-bold text-sm ${textPrimary}`}>₦180,000</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Target Index Unit</span> <p className={`font-bold text-sm ${textPrimary}`}>Hydration &lt; 20%</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Baseline Cost</span> <p className={`font-bold text-sm ${textPrimary}`}>₦6,000 / ssn</p></div>
                                </div>
