...
import { Wind, ChevronDown, CheckCircle2, AlertTriangle, ExternalLink, Phone, Mail, HelpCircle, ArrowRight } from 'lucide-react';
import { useUser } from '../../context/UserContext';

export default function EvaporationInsurance() {
    ...
    const highlights = [
        "Multi-parameter drought detection",
        "Captures hidden drought stress",
        "High-evaporation zones covered",
        "More precise than rainfall-only formulas"
    ];

    const benefits = [
        "Captures invisible ambient drought stress",
        "Highly optimized for high-value/specialty crops",
        "Maintains long-term yield production stability",
        "Significantly more accurate than plain rainfall metrics"
    ];

    const limitations = [
        "More complex architecture rules to interpret",
        "Higher base premiums due to advanced data monitoring",
        "Requires highly comprehensive satellite/sensor telemetry datasets",
        "Limited regulatory microinsurance provider availability"
    ];
        ...
        >
            ...
            <motion.div variants={itemVariants} className="text-center max-w-2xl mx-auto pt-6 mb-4">
                ...
                <h1 className={`text-4xl font-black tracking-tight mb-3 ${textPrimary}`}>
                    Evapotranspiration (ET) Insurance
                </h1>
                <p className={`text-sm tracking-wide font-medium ${textSecondary}`}>
                    Advanced Multi-Parameter Crop Water Stress Protection
                </p>
                ...
                <div className={`md:col-span-2 border rounded-3xl p-8 flex flex-col justify-between transition-all ${cardBg}`}>
                    <div>
                        ...
                        <p className={`text-sm leading-relaxed font-normal ${textSecondary}`}>
                            This advanced atmospheric model isolates actual crop water stress by compounding surface temperatures, solar radiation indices, and subterranean moisture telemetry. Instead of relying solely on baseline rainfall data, it generates an authentic representation of plant transpiration requirements and total system evaporation loss.
                        </p>
                    </div>
                </div>
                ...
                <div className={`rounded-3xl border-2 p-8 flex flex-col justify-between transition-all ${
                    ...
                    <div>
                        ...
                        <p className={`text-xs leading-relaxed font-medium ${theme ? "text-slate-700" : "text-slate-300"}`}>
                            Engineered for microclimates where precipitation volume appears stable on paper, but high evaporation velocities still trigger deep crop dehydration. This ensures precision data payouts before visible tissue damage materializes.
                        </p>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-blue-500 mt-4">
                        ET Matrix Engaged <ArrowRight size={10} />
                    </div>
                </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: "Trigger Metric", val: "ET Stress Threshold" },
                    { label: "Payout Velocity", val: "3–7 Days" },
                    { label: "Product Class", val: "Multi-Index Based" },
                    { label: "Base Premium", val: "₦9,500 / ssn" }
                ].map((metric, idx) => (
                        ...
                        >
                            <div className={`p-8 border-t text-xs space-y-6 ${borderStyle} ${theme ? "bg-white" : "bg-slate-950/20"}`}>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Sample Crop Scope</span> <p className={`font-bold text-sm ${textPrimary}`}>2.0 Ha (High-Value)</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Guaranteed Cap</span> <p className={`font-bold text-sm ${textPrimary}`}>₦300,000</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Trigger Rule</span> <p className={`font-bold text-sm ${textPrimary}`}>ET Stress Breached</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Telemetry Stream</span> <p className={`font-bold text-sm ${textPrimary}`}>Solar / Thermal / Soil</p></div>
                                </div>
