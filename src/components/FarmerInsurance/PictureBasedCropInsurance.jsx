...
import { Camera, ChevronDown, CheckCircle2, AlertTriangle, ExternalLink, Phone, Mail, HelpCircle, ArrowRight } from 'lucide-react';
import { useUser } from '../../context/UserContext';

export default function PictureBasedCropInsurance() {
    ...
    const highlights = [
        "On-demand digital photo documentation",
        "Significantly minimizes index basis risk",
        "Ensures hyper-granular, fair asset assessment",
        "Expedites structural indemnity validation cycles"
    ];

    const benefits = [
        "Covers multi-risk losses index models completely miss",
        "Fair, objective settlement through concrete documentation",
        "Fully transparent and trackable digital claim process",
        "Encourages technological agility and innovation in farming"
    ];

    const limitations = [
        "Strictly requires operational smartphone & local internet access",
        "Remote assessor validation bandwidth may experience variance",
        "Digital divide friction across remote, off-grid field areas",
        "Inherently more complex than instant parametric models"
    ];

    const providers = [
        { name: "APA (Agricultural Producers Association)", phone: "+234 800 APA", url: "https://www.apa.org.ng", email: "support@apa.org.ng" },
        { name: "NAIC Digital Insurance (National Agricultural Corp)", phone: "+234 800 NAIC", url: "https://naic.gov.ng", email: "digital@naic.gov.ng" }
    ];
        ...
        >
            ...
            <motion.div variants={itemVariants} className="text-center max-w-2xl mx-auto pt-6 mb-4">
                ...
                <p className={`text-sm tracking-wide font-medium ${textSecondary}`}>
                    Digital-Verified Mobile Indemnity Protection Layer
                </p>
                ...
                <div className={`md:col-span-2 border rounded-3xl p-8 flex flex-col justify-between transition-all ${cardBg}`}>
                    <div>
                        ...
                        <p className={`text-sm leading-relaxed font-normal ${textSecondary}`}>
                            This mechanism utilizes smartphones to efficiently audit and process localized field losses under an optimized indemnity framework. Producers directly document asset degradation via timestamped imagery, allowing verification networks to evaluate fine physical details, minimize regional index errors, and release fair compensation without mandatory layout audits.
                        </p>
                    </div>
                </div>
                ...
                <div className={`rounded-3xl border-2 p-8 flex flex-col justify-between transition-all ${
                    ...
                    <div>
                        ...
                        <p className={`text-xs leading-relaxed font-medium ${theme ? "text-slate-700" : "text-slate-300"}`}>
                            Combines absolute architectural flexibility with rapid field tech. It captures unpredictable localized damage vectors—such as precise disease patterns, pest outbreaks, or localized hail strikes—that coarse satellite telemetry layers can easily miss.
                        </p>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-blue-500 mt-4">
                        Image Telemetry Active <ArrowRight size={10} />
                    </div>
                </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: "Trigger Metric", val: "Photo-Verified Damage" },
                    { label: "Payout Velocity", val: "5–14 Days" },
                    { label: "Product Class", val: "Indemnity / Mobile" },
                    { label: "Base Premium", val: "₦7,500 / ssn" }
                ].map((metric, idx) => (
                        ...
                        >
                            <div className={`p-8 border-t text-xs space-y-6 ${borderStyle} ${theme ? "bg-white" : "bg-slate-950/20"}`}>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Sample Crop Scope</span> <p className={`font-bold text-sm ${textPrimary}`}>2.0 Ha (Multi-Crop)</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Guaranteed Cap</span> <p className={`font-bold text-sm ${textPrimary}`}>₦222,000</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Trigger Method</span> <p className={`font-bold text-sm ${textPrimary}`}>App Photo Upload</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Verification Node</span> <p className={`font-bold text-sm ${textPrimary}`}>Digital Assessor Panel</p></div>
                                </div>
