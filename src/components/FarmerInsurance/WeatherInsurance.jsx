...
import { CloudLightning, ChevronDown, CheckCircle2, AlertTriangle, ExternalLink, Phone, Mail, HelpCircle, ArrowRight } from 'lucide-react';
...
export default function WeatherInsurance() {
    ...
    const highlights = [
        "Automatic trigger on objective data",
        "Simple to understand and verify",
        "Weather station data widely available",
        "Fastest structural payout speed"
    ];

    const benefits = [
        "Easy to understand operational rules",
        "Affordable premiums for smallholders",
        "Widely available through local stations",
        "Works uniformly for almost all crops"
    ];

    const limitations = [
        "Basis risk if regional weather station data differs from your field",
        "May not match hyper-local individual farm conditions",
        "Strictly limited to measurable, station-tracked weather events"
    ];

    const providers = [
        { name: "NAIC (Nigerian Agricultural Insurance Corporation)", phone: "+234 800 NAIC", url: "https://naic.gov.ng", email: "info@naic.gov.ng" },
        { name: "ARC RiskView (Macro Weather Analytics Platform)", phone: "+234 803 ARCWX", url: "https://www.arcweather.org/riskview", email: "riskview@arcweather.org" }
    ];
        ...
        >
            ...
            <motion.div variants={itemVariants} className="text-center max-w-2xl mx-auto pt-6 mb-4">
                ...
                <p className={`text-sm tracking-wide font-medium ${textSecondary}`}>
                    Parametric Environmental Boundary Protection Layer
                </p>
                ...
                <div className={`md:col-span-2 border rounded-3xl p-8 flex flex-col justify-between transition-all ${cardBg}`}>
                    <div>
                        ...
                        <p className={`text-sm leading-relaxed font-normal ${textSecondary}`}>
                            This policy is calibrated entirely to objective physical data measured over localized target cycles at specific ground telemetry stations. Rather than auditing individual field loss, the system triggers capital deployment automatically whenever raw baseline observations (such as precipitation thresholds or wind velocity metrics) cross explicit contract parameters.
                        </p>
                    </div>
                </div>
                ...
                <div className={`rounded-3xl border-2 p-8 flex flex-col justify-between transition-all ${
                    ...
                    <div>
                        ...
                        <p className={`text-xs leading-relaxed font-medium ${theme ? "text-slate-700" : "text-slate-300"}`}>
                            The cleanest, most intuitive entrance vector for parametric protection. By strip-mining administrative friction and claims forms out of the equation, it delivers fast liquidity exactly when regional weather charts destabilize.
                        </p>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-blue-500 mt-4">
                        Data-Driven Certainty <ArrowRight size={10} />
                    </div>
                </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: "Trigger Metric", val: "Rain >100mm / <10mm" },
                    { label: "Payout Velocity", val: "24–48 Hours" },
                    { label: "Product Class", val: "Parametric Index" },
                    { label: "Base Premium", val: "₦7,000 / ssn" }
                ].map((metric, idx) => (
                        ...
                        >
                            <div className={`p-8 border-t text-xs space-y-6 ${borderStyle} ${theme ? "bg-white" : "bg-slate-950/20"}`}>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Sample Crop Scope</span> <p className={`font-bold text-sm ${textPrimary}`}>2.5 Ha (Groundnut)</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Guaranteed Cap</span> <p className={`font-bold text-sm ${textPrimary}`}>₦200,000</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Trigger Rule</span> <p className={`font-bold text-sm ${textPrimary}`}>&gt; 100mm / &lt; 10mm</p></div>
                                    <div><span className={`block text-[9px] font-black uppercase tracking-wider mb-1 ${textMuted}`}>Primary Metric</span> <p className={`font-bold text-sm ${textPrimary}`}>Rainfall Volatiles</p></div>
                                </div>
